...
public class EmailExtractor {
    ...
    private List<String> getEmailFromSmallText(String part){
        ...
        ret.add(candidate);
        ret = emailValidator.validate(ret);

        return ret;
    }
    private List<String> getEmailDeepAnalysis(String text){
        List<String> ret = new ArrayList<>();
    ...
    private List<String> getEmail(String text){
        String[] tokens = text.split("\\s+");
        List<String> ls = Arrays.asList(tokens);

        List<String> filtered = emailValidator.validate(ls);
    ...
    private List<String> getEmail(PDDocument doc){
        String text = null;
        try{
            PDFTextStripper strip = new PDFTextStripper();
            text = strip.getText(doc).toLowerCase();
        ...
        else return getEmail(text);
    }
    private List<String> getEmails(File file){
        try(PDDocument doc = PDDocument.load(file)) {
            if(doc == null || doc.isEncrypted()) return null;
            else return getEmail(doc);
    ...
    private List<String> getEmails(List<File> files){
        List<String> ret = new ArrayList<>();
        for(File file : files) {
            if(!FilenameUtils.getExtension(file.getAbsolutePath()).toLowerCase().equals("pdf")) {differentExt++;continue;}

            List<String> get = getEmails(file);
            if(get != null)
                ret.addAll(get);
